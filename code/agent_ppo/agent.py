...
class Agent(BaseAgent):
    ...
    def observation_process(self, env_obs):
        """Convert raw env_obs to ObsData (1500D feature + legal action mask).

        将原始 env_obs 转换为 ObsData（1500D 特征 + 合法动作掩码）。
        """
    ...
    def action_process(self, act_data, is_stochastic=True):
        ...
        仅在低电量且需要判断是否规则接管时，才惰性计算规则动作。
        """
        model_action = act_data.action if is_stochastic else act_data.d_action
        model_action = int(model_action[0])

        battery = float(getattr(self.preprocessor, "battery", 0.0))
        battery_max = max(float(getattr(self.preprocessor, "battery_max", 1.0)), 1.0)
        battery_ratio = battery / battery_max

        low_battery_threshold = float(getattr(self.preprocessor, "low_battery_threshold", 0.30))
        very_low_battery_threshold = float(getattr(self.preprocessor, "very_low_battery_threshold", 0.15))
        ...
        if final_action == model_action and battery_ratio < low_battery_threshold:
            ...
            if use_rule_guide > 0.5 and rule_action >= 0:
                # 极低电量：强制规则接管
                if battery_ratio < very_low_battery_threshold:
        ...
        assert isinstance(self.last_action, int), f"action type error: {type(self.last_action)}"
        assert 0 <= self.last_action < 8, f"invalid final action: {self.last_action}"
    ...
    def _legal_soft_max(self, logits, legal_action):
        ...
        合法动作掩码下的 softmax。
        当掩码导致无可行动作（全 0）或数值异常时，回退为在合法维上均匀分布，避免除零。
        """
        la = np.asarray(legal_action, dtype=np.float32).reshape(-1)
        if la.size != 8:
            la = np.ones(8, dtype=np.float32)

        _w, _e = 1e20, 1e-5
        tmp = np.asarray(logits, dtype=np.float64).reshape(-1)[:8]
        if tmp.size < 8:
            tmp = np.pad(tmp, (0, 8 - tmp.size))

        tmp = tmp - _w * (1.0 - la)
        tmp_max = np.max(tmp)
        tmp = np.clip(tmp - tmp_max, -_w, 1.0)
        tmp = (np.exp(tmp) + _e) * la
        denom = np.sum(tmp) * 1.00001
        if not np.isfinite(denom) or denom <= 1e-12:
            n = float(np.sum(la))
            if n > 0.0:
                return (la / n).astype(np.float32)
            return np.full(8, 1.0 / 8.0, dtype=np.float32)
        out = (tmp / denom).astype(np.float32)
        return out
    ...
    def _legal_sample(self, probs, use_max=False):
        ...
        """
        p = np.asarray(probs, dtype=np.float64).reshape(-1)[:8]
        if p.size < 8:
            p = np.pad(p, (0, 8 - p.size))
