...
class Preprocessor:
    ...
    def _get_npc_keepout_anchors(self):
        """每个官方机：当前格中心 + 一步运动预测格中心，供距离保留区判断。"""
        anchors = []
        if not self.npc_positions:
            return anchors

        lasts = self.last_npc_positions if self.last_npc_positions else []
        seen = set()

        for px, pz in self.npc_positions:
            px, pz = int(px), int(pz)
            if (px, pz) not in seen:
                seen.add((px, pz))
                anchors.append((float(px), float(pz)))

            if lasts:
                lx, lz = min(lasts, key=lambda q: (q[0] - px) ** 2 + (q[1] - pz) ** 2)
                vx = px - int(lx)
                vz = pz - int(lz)
                sx = int(np.clip(vx, -1, 1))
                sz = int(np.clip(vz, -1, 1))
                if sx != 0 or sz != 0:
                    ax, az = px + sx, pz + sz
                    if (ax, az) not in seen:
                        seen.add((ax, az))
                        anchors.append((float(ax), float(az)))

        return anchors

    @staticmethod
    def _min_euclidean_dist_to_anchors(gx, gz, anchors):
        if not anchors:
            return 999.0
        gx, gz = float(gx), float(gz)
        return float(min(np.sqrt((gx - ax) ** 2 + (gz - az) ** 2) for ax, az in anchors))

    def _mask_actions_entering_npc_tile(self, legal):
        """按距离保留区：若动作落点与任一 NPC 锚点过近则禁止（无 8 邻格离散扩圈）。"""
        if not self.npc_positions:
            return legal

        anchors = self._get_npc_keepout_anchors()
        if not anchors:
            return legal

        hx, hz = self.cur_pos
        r = float(self.NPC_KEEPOUT_RADIUS)
        out = list(legal)
        for act, (dx, dz) in enumerate(self._get_action_dirs()):
            if out[act] != 1:
                continue
            gx, gz = hx + dx, hz + dz
            if self._min_euclidean_dist_to_anchors(gx, gz, anchors) < r:
                out[act] = 0
        return out
    ...
    def pb2struct(self, env_obs, last_action):
        ...
        if observation is None:
            raise ValueError("env_obs['observation'] is None")
        frame_state = observation.get("frame_state")
        if frame_state is None:
            raise ValueError("observation['frame_state'] is None")
        env_info = observation.get("env_info") or {}
        heroes_raw = frame_state.get("heroes")
        if isinstance(heroes_raw, (list, tuple)):
            if len(heroes_raw) == 0:
                raise ValueError("frame_state['heroes'] is empty list")
            hero = heroes_raw[0]
        else:
            hero = heroes_raw
        if hero is None or not isinstance(hero, dict):
            raise ValueError("frame_state['heroes'] must be dict or non-empty list of dicts")
        ...
        # Legal actions: map + charger；再按与 NPC 锚点的欧氏距离剔除过近的落点（见 NPC_KEEPOUT_RADIUS）
        map_legal = self._build_collision_free_legal_action()
        masked = self._mask_actions_entering_npc_tile(map_legal)
        if sum(masked) == 0:
            # NPC 保留区与墙完全叠死时避免全零掩码（否则策略 softmax 除零）
            self._legal_act = map_legal if sum(map_legal) > 0 else [1] * 8
        else:
            self._legal_act = masked
    ...
    def _get_dirt_approach_reward(self):
        ...
        低电量时关闭，避免与回充目标冲突。
        官方机器人很近时不鼓励“贴污渍”，避免与避碰抢梯度。
        """
        if self._is_low_battery_mode():
            return 0.0

        if self.nearest_npc_dist < 2.5:
            return 0.0
    ...
    def _get_npc_avoid_reward(self):
        """官方机器人避碰：远距离软惩罚 + 近距离强惩罚。"""
        if not self.npc_positions:
            return 0.0

        safe_npc_dist = 3.5
        danger_npc_dist = 1.2
        collision_npc_dist = 0.85

        d = float(self.nearest_npc_dist)
        if d <= collision_npc_dist:
            return -2.0
        if d < danger_npc_dist:
            span = max(danger_npc_dist - collision_npc_dist, 1e-6)
            t = (danger_npc_dist - d) / span
            return float(-0.9 - 1.1 * min(t, 1.0))
        if d < safe_npc_dist:
            return float(-0.22 * ((safe_npc_dist - d) ** 1.15))
        return 0.0

    def _get_npc_closing_penalty(self):
        """本步若明显朝最近官方机器人靠近，额外惩罚（与距离势场叠加）。"""
        if not self.npc_positions or self.step_no <= 0:
            return 0.0

        if self.nearest_npc_dist >= 4.0:
            return 0.0

        delta = float(self.last_nearest_npc_dist - self.nearest_npc_dist)
        if delta <= 0.03:
            return 0.0

        closeness = max(0.0, (4.0 - float(self.nearest_npc_dist)) / 4.0)
        speed = min(delta / 0.6, 2.0)
        return float(-0.28 * closeness * speed)
    ...
    def reward_process(self):
        ...
        npc_closing_penalty = self._get_npc_closing_penalty()
        ...
        obstacle_reward = npc_avoid_reward + npc_closing_penalty
        charger_reward = (
        ...
        reward_info = {
            ...
            "npc_closing_penalty": float(npc_closing_penalty),
