...
SampleData = create_cls(
    "SampleData",
    obs=Config.DIM_OF_OBSERVATION,      # 1500D feature vector / 特征向量
    legal_action=Config.ACTION_NUM,     # 8D legal action mask / 合法动作掩码
    act=1,                              # action index / 执行的动作

    reward=Config.VALUE_NUM,            # 1D total reward / 总奖励
    cleaning_reward=Config.VALUE_NUM,   # 1D cleaning reward / 清扫奖励
    dirt_approach_reward=Config.VALUE_NUM,  # 1D dirt approach reward / 接近污渍奖励
    obstacle_reward=Config.VALUE_NUM,   # 1D obstacle safety reward / 障碍物安全惩罚
    charger_reward=Config.VALUE_NUM,    # 1D charger reward / 低电量接近充电桩奖励
    step_penalty=Config.VALUE_NUM,      # 1D time penalty / 时间惩罚
